...
def evaluation(args):
    ...
    with open(e2e_file, "w", encoding="utf-8") as f:
        # Save scores for all test queries
    ...
    aggregated_e2e_file = os.path.join(LOG_DIR, f"aggregated_end_to_end_{method}_doc_n{doc_n}.json")
    ...
    for metric_name in retrieval_metrics: # Iterate using the base names
        ...
        if num_queries_with_metric < 2:
            print(f"  Skipping correlation for {metric_name}: fewer than 2 queries with this metric ({num_queries_with_metric} found).")
            ...
            print(f"  Skipping correlation for {metric_name}: insufficient variance in scores ({num_queries_with_metric} pairs).")
            spearman_corr, spearman_p = float('nan'), float('nan')
            kendall_corr, kendall_p = float('nan'), float('nan')
        else:
