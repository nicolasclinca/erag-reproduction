...
End-to-end evaluation (FiD) for each k in k_values, logging scores to CSV.

For each run CSV file in --input_folder (e.g., ../input_runs/nq) it produces:
...
- <input_filename>_mean_downstream.csv
    columns: score, k
  (one row per k: mean score over all queries)

Input
-----
- run CSV file with at least the columns:
    query_id, doc_id, score, run_id
  (any extra columns like relevance are ignored)

- KILT dataset (.jsonl) for:
    query_id -> query_text
    query_id -> list of gold answers

- JSONL collection for:
    doc_id -> document contents

- FiD-T5 model for generation.

Notes
----
- The ranking is reconstructed by sorting by descending score and deduplicating doc_id per query_id.
- If the dataset contains duplicate query_text for different query_id, generation is deduplicated
  by query_text (as in build_erag_qrels.py), but the downstream metric is computed for each query_id.
...
METRICS = {
    ...
    "f1": f1_metric,
    "accuracy": exact_match_metric,  # in this project it coincides with EM
}
...
def list_candidate_run_csv_files(input_folder: str) -> List[str]:
    """
    Lists CSV files that look like run files.
    Excludes output files generated by previous scripts.
    """
    files: List[str] = []
...
def load_run_grouped_csv(
    ...
    """
    Returns:
      retrieval_results: {query_id: [doc_id1, doc_id2, ...]} sorted by score desc
      qids: set(query_id)
      docids: set(doc_id)
      run_id: first non-empty run_id (fallback: "run")
    """
    ...
    for qid, pairs in per_q.items():
        ...
        # dedup doc ids while preserving order
        seen: Set[str] = set()
        out: List[str] = []
...
def load_collection_contents_by_line_scan(collection_path: str, needed_docids: Set[str]) -> Dict[str, str]:
    """
    Slow fallback: scan and capture only the required lines (numeric doc_id = line number).
    """
...
def _build_generator_input_from_ids(
    ...
    """
    Converts {query_id: [doc_id,...]} into {query_text: [doc_text,...]} for the generator.
    Deduplicates by query_text (same generation).
    """
